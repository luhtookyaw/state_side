...
EXAMPLE_C_ALLIANCE= {
    ...
    'score': '4',
    'reason': 'The client expressed his fear of marriage and love during the conversation, and the counselor had some in-depth discussions on this issue. Although the counselor and the client did not explicitly mention the goals, they have been actively solving the problems raised by the client and exploring ways to meet their needs. At the same time, the client expressed satisfaction with the results achieved.',
}

C_ALLIANCE_SYSTEM_PROMPT = """
Below is a psychological counseling dialogue between a counselor and a client. 
As a third party, please read the conversation and score the following question from 1 to 5 based on the definitions.

# Questions
Goal
Q1. There are doubts or a lack of understanding about what participants are trying to accomplish in therapy.
Q1 definition:
1 = The counselor and the client have clearly identified different goals, and there are disagreements in the order of issues and solutions in therapy. This inconsistency may lead the client to express strong dissatisfaction with the overall counseling process and goals, possibly mentioning the reasons for participating in therapy. This could further trigger a negative reaction from the counselor. At this stage, it seems challenging for both parties to find common ground, making the therapeutic process difficult.
...
4 = The counselor and the client do not explicitly mention the goals but are working towards a common objective. The counselor addresses the client’s concerns immediately and adjusts the therapeutic process to meet the client’s needs. The client is satisfied with the progress made.
...
5 = The client is satisfied and excited about the counselor’s methods and approach to problemsolving. His/her performance in therapy is highly positive, possibly suggesting suggestions to further advance therapy tasks. Overall, the client is content with therapy work, and their interaction demonstrates a high level of cooperation and enthusiasm.
...
# Instruction
List the corresponding evidence found in the conversation and provide analysis.  

# Output Format
Score the conversation of the counselor and client based on the above definitions. Return the score in the following JSON format , with double quotes around keys and string values as illustrated below::
{example}
...
def evaluate_alliance(
    ...
    raw = model_call(prompt, 1400)
    return parse_alliance_response(raw)
